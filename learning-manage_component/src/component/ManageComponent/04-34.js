...
function withSomething(InputComponent) {
  return class OutputComponent extends InputComponent {
    ...
    render() {
      debugger;
      this.MyComponentFunction(); //POINT A: 상속한 InputComponent에 있는 함수를 호출
...
class MyComponent extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <p>코드 4-34 클래스 상속을 이용한 고차 컴포넌트</p>
        <p>
          * 입력된 컴포넌트로 만들어진 인스턴스의 "속성값, 상탯값, 생명주기
          메서드, 기타 멤버 변수/ 메서드"에 접근할 수 있다.
        </p>
      </div>
    );
  }
