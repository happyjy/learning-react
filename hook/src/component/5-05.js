...
/*
  ...
  * 리액트 팀에서는 기존의 생명 주기 메서드에 문제가 있다고 판다. 
    ...
    - useEffect 훅을 이용하면 비슷한 기능을 한 곳으로 모을 수 있어 가동성 좋아진다.
  ...
  return (
    <div>
      <div>
        ...
        <p>
          {" "}
          * ⭐️ 클래스형 컴포넌트의 각 생명 주기 메서드에 일대일로 대응하는 훅이
          있는 것은 아니다{" "}
        </p>
        <p> * 리액트 팀에서는 기존의 생명 주기 메서드에 문제가 있다고 판다. </p>
        <p>
          {" "}
          - ⭐️ 한가지 문제는 서로 연관성이 없는 여러 기능이 하나의 생명주기
          메서드에 섞이게 된 다는 점
        </p>
        <p>
          {" "}
          - useEffect 훅을 이용하면 비슷한 기능을 한 곳으로 모을 수 있어 가동성
          좋아진다.
        </p>
        <p> </p>
        <p> * 예제 설명 </p>
        <p>
          {" "}
          - API를 호출하는 기능과 이벤트 처리 함수를 등록하고 해제하는 기능를
          각각 훅으로 구현
        </p>
        <p>
          {" "}
          - 두 기능을 하나로 합쳤을 때 함수형 컴포넌트와 클래스형 컴포넌트에서
          어떤 모습이 되는지 예제 코드 5-5, 5-6에서 비교{" "}
        </p>
        <p></p>
        <p>
          {" "}
        ...
        <p>
          {" "}
          - 클래스형 컴포넌트의 componentDidMount, componentDidUpdate 양쪽
          메서드에 추가하면 가은 기능을 하게 된다.
        </p>
        <p> * ⭐️ 버튼을 누르면? </p>
        <p>
          {" "}
