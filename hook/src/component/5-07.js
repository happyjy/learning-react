    ...
    return (
      <div>
        <div>
          ...
          <p>
            {" "}
            두번째 렌더링 후부터는 componentDidUpdate 메서드가 호출 되므로 API와
            통신하는 코드를 양쪽에서 작성해야한다.
          </p>
          <p>
            {" "}
            * 단 componentDidUpdate 메서드에서는 userId가 변경된 경우에만 API를
            호출{" "}
          </p>
          <p>
            {" "}
            * 이 처럼 클래스형 컴포넌트에서는 중복된 코드가 여러 생명주기
            메서드에 흩어져 있는 경우가 많다.
          </p>
        </div>
        <hr></hr>
